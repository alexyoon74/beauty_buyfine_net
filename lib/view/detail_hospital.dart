import 'package:beauty_buyfine_net/components/detail_page_func.dart';
import 'package:beauty_buyfine_net/controller/detail_controller.dart';
import 'package:flutter/material.dart';
import 'package:get/get.dart';

class DetailHospital extends GetView<DetailController> {
  final int uid;
  DetailHospital({
    super.key,
    required this.uid,
  });

  final DetailController detailHospitalController = Get.find<DetailController>(
    tag: 'DetailHospitalController',
  );
  final DetailPageFunc detailPageFunc = DetailPageFunc();

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Hospital (uid=$uid)'),
      ),
      body: const Center(
        child: Column(
          children: [],
        ),
      ),
    );
  }
}
